...
from voice_locale import is_spanish_only, normalize_spanish_transcript


def _default_speech_model() -> str:
    """Spanish sessions default to universal-streaming-multilingual."""
    explicit = (os.getenv("ASSEMBLYAI_SPEECH_MODEL") or "").strip()
    if explicit:
        return explicit
    if is_spanish_only():
...
class AssemblyAISTT:
    def __init__(
        ...
        self.speech_model = speech_model or _default_speech_model()
        ...
        if _is_whisper_model(self.speech_model):
            # whisper-rt: no `language` query param (auto-detect only)
            self.streaming_language = None
            self.language_detection = True
        elif "multilingual" in self.speech_model.lower():
                        ...
                        try:
                            ...
                            if message_type == "Begin":
                                ...
                                if end_of_turn:
                                    ...
                                    if final:
                                        if is_spanish_only():
                                            final = normalize_spanish_transcript(final)
                                        yield STTOutputEvent.create(final)
                                else:
                                    partial = utterance or transcript
                                    if partial:
                                        if is_spanish_only():
                                            partial = normalize_spanish_transcript(
                                                partial
                                            )
                                ...
                                print(f"AssemblyAISTT Error: {err}")
                                self._ws = None
                                break
    ...
    async def _ensure_connection(self) -> WebSocketClientProtocol:
        ...
        # whisper-rt: only base params + language_detection (extra turn params can break STT)
        if not _is_whisper_model(self.speech_model):
        ...
        print(
            f"AssemblyAISTT connected: model={self.speech_model} "
            f"language={self.streaming_language or '(auto)'} "
            f"lang_detect={self.language_detection}"
        )
