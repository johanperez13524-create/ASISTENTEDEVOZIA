...
# ── Kitchen Display System event bus ───────────────────────────────────────

class KDSBus:
    """Fan-out bus: one asyncio.Queue per connected /ws/kitchen client."""
...
def add_to_cart(
    ...
    """Añade al carrito. product_id: id del menú (meat-turkey) o nombre en español (pavo, tomate)."""
    with restaurant_db.db_session() as conn:
...
def confirm_order(runtime: ToolRuntime[VoiceSessionContext]) -> str:
    ...
    with restaurant_db.db_session() as conn:
        ...
        if not res["ok"]:
            return restaurant_db.format_error_tool_result(res["error"])
        oid = res["order_id"]
        short_id = oid[-4:].upper()
        msg = (
            f"Pedido confirmado. Tu código de recogida es el <spell>{short_id}</spell>. "
...
system_prompt = f"""
Tu eres el asistente de voz de un restaurante de sandwiches en España.
Objetivo: tomar el pedido y confirmarlo.
SOLO español: nunca respondas en inglés. El cliente siempre habla en español.

Reglas del menú y herramientas:
- No inventes productos. Usa get_menu_stock si no sabes qué hay o antes de agregar.
- add_to_cart acepta product_id (meat-turkey) o nombre en español (pavo, pollo, tomate).
- Si la transcripción suena en inglés por error de voz, interpreta en español:
  "power"/"turkey" → pavo (meat-turkey), "tomato" → tomate (veg-tomato), "chicken" → pollo.
- Nunca digas que no hay "sandwich the power": es "sándwich de pavo".
...
- Al confirmar el pedido, NUNCA leas el 'order_id' largo (el identificador con guiones). Lee ÚNICAMENTE el código corto de recogida proporcionado.
        ...
        if event.type == "stt_output":
            ...
            async for message, metadata in stream:
                ...
                if isinstance(message, ToolMessage):
                    ...
                    # KDS: broadcast new order when confirm_order succeeds
                    if tool_name == "confirm_order" and text:
                        try:
                            obj = json.loads(text)
                            if obj.get("kind") == "order" and obj.get("order"):
                                oid = obj["order"].get("order_id", "")
                                if oid:
                                    with restaurant_db.db_session() as conn:
                                        full = restaurant_db.get_order_with_lines(conn, oid)
                                    if full:
                                        await kds_bus.publish({"type": "order_new", "order": full})
                        except Exception:
                            pass
...
def api_get_active_orders():
    """Return active (non-ready) orders for KDS pre-load on reconnect."""
    with restaurant_db.db_session() as conn:
        return restaurant_db.get_active_orders(conn)


@app.patch("/api/orders/{order_id}/status")
async def api_update_order_status(order_id: str, body: OrderStatusBody):
    """Update order status and broadcast to all KDS clients."""
    with restaurant_db.db_session() as conn:
        res = restaurant_db.update_order_status(conn, order_id, body.status)
    if not res["ok"]:
        raise HTTPException(status_code=404, detail=res["error"])
    await kds_bus.publish(
        {"type": "status_update", "order_id": order_id, "status": body.status}
    )
    return res
...
async def kitchen_websocket_endpoint(websocket: WebSocket):
    """Real-time feed for Kitchen Display System clients."""
    await websocket.accept()
    queue = kds_bus.subscribe()
    try:
        # Send current active orders on connect so the display is never blank
        with restaurant_db.db_session() as conn:
            active = restaurant_db.get_active_orders(conn)
        await websocket.send_json({"type": "order_list", "orders": active})
        # Forward KDS bus events to this client
        while True:
            try:
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    try:
        async for event in output_stream:
            print(f"Event to client: {event.type}")
